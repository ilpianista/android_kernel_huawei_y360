...
#include <linux/reboot.h> //brian-add
#include <mach/mt_boot.h>//brian-add
 int bootbuf;//brian-add
#endif
	...
	int (*fill_super)(struct super_block *, void *, int))
{
	...
	if (s->s_root) {
		...
		if (error) {
			...
			//brian-add-s
				if (0 == get_boot_mode())
			if (!strcmp(dev_name, "/emmc@usrdata" ) )
			{//if ((s->s_flags & MS_RDONLY)&&(!strcmp(dev_name, "/emmc@usrdata" )))
			if ((bootbuf!=0x5a5a5a5a ))
				{
				*((int*)0xF900ff00) = 0x5a5a5a5a;
			      kernel_restart(NULL); //brian-debug
				}
			}
			//brian-add-e
	           #endif
